...
import { HandStatus, IBoughtInsurance, IDeclinedInsurance, IPlayer } from "blackjack-types";

export class PlayerState implements ToClientJSON<IPlayer> {
    private _hands: { [handId: string]: PlayerHandState } = {}

    money = 1000

    ready = false

    // TODO can we just return Object.values here? Probably more ergonomic for consumers. Can we make the return value readonly too?
    get hands(): { [handId: string]: PlayerHandState } {
        return this._hands
    }

    get hasHand(): boolean {
        return Object.values(this._hands).length > 0
    }
    ...
    toClientJSON(): IPlayer {
        const hands = Object.fromEntries(
            Object.entries(this._hands)
                .map(([handId, hand]) => 
                    [handId, hand.toClientJSON()]
                )
        )
